...
func TestJWTAuth_WithErrorHandler_NilService(t *testing.T) {
	...
	handler := JWTAuth(nil, WithErrorHandler(func(w http.ResponseWriter, _ *http.Request, _ error, status int) {
...
func TestJWTAuth_WithErrorHandler_NoToken(t *testing.T) {
	...
	svc := newTestService(t, nil)
	var capturedStatus int
	handler := JWTAuth(svc, WithErrorHandler(func(w http.ResponseWriter, _ *http.Request, _ error, status int) {
		capturedStatus = status
		w.WriteHeader(status)
	}))(http.HandlerFunc(func(_ http.ResponseWriter, _ *http.Request) {
		t.Error("handler should not be called")
...
func TestJWTAuth_WithErrorHandler_InvalidToken(t *testing.T) {
	...
	handler := JWTAuth(svc, WithErrorHandler(func(w http.ResponseWriter, _ *http.Request, err error, status int) {
		capturedStatus = status
		capturedErr = err
		w.WriteHeader(status)
	}))(http.HandlerFunc(func(_ http.ResponseWriter, _ *http.Request) {
